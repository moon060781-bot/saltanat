    ...
    <div class="dashboard-grid">
      <section class="card">
        ...
        <div class="card-body">
          ...
          <?php if (!$status['clean']): ?><div class="dirty-diagnostic"><strong>Sync block کرنے والی repository entries</strong><?php if ($status['dirtyEntries']): ?><?php foreach ($status['dirtyEntries'] as $entry): ?><code><?= e($entry) ?></code><?php endforeach; ?><?php else: ?><span>Git status detail دستیاب نہیں ہے؛ cPanel filesystem permission check درکار ہے۔</span><?php endif; ?></div><?php endif; ?>
        </div>
      </section>
      <aside class="card">
